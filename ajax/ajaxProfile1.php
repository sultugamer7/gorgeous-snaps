<?php
	...
	// Checking if profile of user matches with the logged in user
	if($username == $username1){
		$div .= '<a class="btn btn-dark btn-sm shadow" href="editprofile.php">Edit Profile</a>';
	}else{
		$sql3 = "SELECT * FROM followers WHERE uid=$id1 AND followerID=$id";
		$result3 = $con->query($sql3);
        if ($result3->num_rows > 0) {
        	$div .= '
                <form name="unfollow" method="post">
                	<input type="button" id="FollowUnfollow" value="Unfollow" style="width: 75px;" class="btn btn-sm btn-outline-primary shadow" onclick="return confirmation();"/>
                </form>
            ';
        }else{
            $div .= '
                <form name="follow" method="post">
                	<input type="button" id="FollowUnfollow" value="Follow" style="width: 75px;" class="btn btn-sm btn-primary shadow" onclick="ajaxProfile2();"/>
                </form>
            ';
        }
    }

    // Designing division according to the user profile
    if($username == $username1){
	...
	$div .= '
		<span>
			<a type="button" class="btn btn-link btn-sm" href="#profile2" style="background: blueviolet;">
			';
				if($totalPosts == 1){
	...
	$div .= '
        ...
        <span style="padding-left: 20px;">
			<a type="button" class="btn btn-link btn-sm" data-toggle="modal" data-target="#followerModal" style="background: blueviolet;">
			';
				if($totalFollowers == 1){
	...
	$div .= '
        ...
        <span style="padding-left: 20px;">
            <a type="button" class="btn btn-link btn-sm" data-toggle="modal" data-target="#followingModal" style="background: blueviolet;">
		        	...
		        	if($result5->num_rows > 0){
		        		while($row = $result5->fetch_assoc()){
		        			...
		        			if($result6->num_rows > 0){
		        				...
		        				";
		        				if($follProfilePic == ""){
					                $div .= "<img src='img/16.jpg' class='imagefit2'>";
					            }else{
					                $div .= "<img src='$follProfilePic' class='imagefit2'>";
					            }
					            $div .= "
					            		</div>
