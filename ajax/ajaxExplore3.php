<?php
	...
	$div3 = '';
                $sql = "SELECT * FROM posts ORDER BY comments DESC LIMIT 9";
                      ...
                      <div class="overlay">
                        ...
                        <div class="row" style="margin-top: 21px; height: 40px; background: rgba(0,0,0,0.6);">
                        	...
                        	if ($con->query($sql2)->num_rows > 0) {
                        		$div3 .= '
                        			<input type="button" id="Unlike" title="Liked" class="btn btn-sm btn-outline-light" value="❤" style="background: rgba(255,255,255,0.6); margin-top: 4.1px; width: 55px;" onclick="ajaxExplore3_3('.$imageId1.','.$uid.','.$NoOfLikes.')">
                        		';
                        	}else{
                        		$div3 .= '
                        			<input type="button" id="Like" title="Like" class="btn btn-sm btn-outline-light" value="👍" style="background: rgba(255,255,255,0.6); margin-top: 4.1px; width: 55px;" onclick="ajaxExplore3_2('.$imageId1.','.$uid.','.$NoOfLikes.')">
                        		';
                        	}
                        	$div3 .= '
                        	</div>
                        	<div class="col-md-6">
                        		<input type="text" id="newCommenttt'.$imageId1.'" placeholder="Add a comment... (Max 100 characters)" style="margin-left: -30px; margin-top: 4px; width: 215px; height: 31px; font-size: 11px; overflow-y: hidden; padding-left: 4px;     background: rgba(0,0,255,0.11); color: whitesmoke;" maxlength="100" class="form-control">
                        	</div>
                        	<div class="col-md-3">
                        		<input type="button" id="Comment" class="btn btn-sm btn-outline-light" title="Comment" value="💬" style="background: rgba(255,255,255,0.6); margin-top: 4.1px; width: 55px;" onclick="ajaxExplore3_4('.$imageId1.','.$uid.','.$NoOfComments.','.$imageId1.')">
                        	</div>
                        </div>
